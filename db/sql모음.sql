...
SELECT ROWNUM, GOODS_NO, GOODS_TITLE, THUMBNAIL
FROM (SELECT G.GOODS_NO, G.GOODS_TITLE,
		(SELECT FILE_PATH||USER_NO||'/'||FILE_NAME FROM "files" F
		WHERE F.GOODS_NO = G.GOODS_NO
		AND FILE_ORDER = 1) THUMBNAIL
FROM "goods_board" G
LEFT JOIN (SELECT * FROM "files"
			WHERE FILE_ORDER = 1) TF ON (G.GOODS_NO = TF.GOODS_NO) 
ORDER BY SELL_ENROLL_DT DESC)RG
WHERE ROWNUM <= 5
;

SELECT GOODS_NO, GOODS_TITLE, THUMBNAIL
FROM (SELECT ROWNUM, GOODS_NO, GOODS_TITLE, THUMBNAIL
		FROM (SELECT G.GOODS_NO, G.GOODS_TITLE,
				(SELECT FILE_PATH||USER_NO||'/'||FILE_NAME FROM "files" F
				WHERE F.GOODS_NO = G.GOODS_NO
				AND FILE_ORDER = 1) THUMBNAIL
